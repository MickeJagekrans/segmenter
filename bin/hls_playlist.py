...
def build_master(source_path):
    ...
    for i, s in enumerate(streams[0]):
        entry = AUDIO_ENTRY_FORMAT.format(i, s['tags']['language'], s['max_bit_rate'])
        master.append(entry)
    ...
    for i, s in enumerate(streams[1]):
        ...
        bandwidth = int(bit_rate) + int(streams[0][i]['max_bit_rate'])
...
def get_streams(source_path):
    ...
    audio = sorted([s for s in streams if s['codec_type'] == 'audio'], key=lambda k: int(k['max_bit_rate']))
...
def get_duration(source_path):
    ...
    output = json.loads(check_output(cmd.split()))
